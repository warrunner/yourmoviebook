...
def searchFunction():
    ...
    if genrevalue.get() == 1:
        for getting in range(0, len(totalgenre), 2):
            if totalgenre[getting].get() == 1:
                ...
                selectedgenre.append(totalgenre[getting+1])
        gen = 1
    ...
    if languagevalue.get() == 1:
        for gettin in range(0,len(totallang), 2):
            if totallang[gettin].get() == 1:
                ...
                selectedlanguage.append(totallang[gettin+1])
        lang = 1
    print(selected)
    print(selectedgenre)
    ...
    result = [0]
    if gen == 1 and lang == 1:
        #executing lang=1 and gen=1
        print('gen1lang1')
        query1 = '''SELECT * FROM MOVIES WHERE (%s) = ANY(genre) and (%s) = language;'''
        
        for a,b in zip(selectedgenre,selectedlanguage):
            conn = psycopg2.connect(dbname= 'moviebook', user='postgres', password='heybuddy', host='localhost', port='5432')
            print(a,b)
            cur=conn.cursor()
            cur.execute(query1,(a,b,))
            result+=cur.fetchall()
            conn.commit()
            conn.close()
    
    elif gen==1 and lang!=1:
        #executing gen=1 and lang!=1
        print('gen1lang0')
        for item in selectedgenre:
            conn = psycopg2.connect(dbname= 'moviebook', user='postgres', password='heybuddy', host='localhost', port='5432')
            cur = conn.cursor()
            query = '''SELECT * FROM MOVIES WHERE (%s) = any(genre);'''
            print(query)
            b = item
            cur.execute(query,(b,))
            row = cur.fetchall()
        ...
        print('gen0lang1')
        #executing gen!=1 and lang=1
        query3 = '''SELECT * FROM MOVIES WHERE (%s) = language'''
        for item in selectedlanguage:
            conn = psycopg2.connect(dbname= 'moviebook', user='postgres', password='heybuddy', host='localhost', port='5432')
            cur=conn.cursor()
            cur.execute(query3, (item,))
            result+=cur.fetchall()
            conn.commit()
            conn.close()
    result.pop(0)
...
def callback(event):
    ...
    conn = psycopg2.connect(dbname= 'moviebook', user='postgres', password='heybuddy', host='localhost', port='5432')
    cur=conn.cursor()
    query = '''SELECT * FROM MOVIES WHERE %s = name or %s = any(actor)'''
    cur.execute(query,(valueGot,valueGot,))
    k = cur.fetchall()
    conn.commit()
    conn.close()
    display_clear()
...
def writingNew():
    ...
    actor = heroEntry.get().lower()
    actor = actor.split(",")
    actor = lst2pgarr(actor)
    genre = genreEntry.get().lower()
    genre = genre.split(",")
    genre=lst2pgarr(genre)
    language = languageEntry.get().lower()
    print(name, actor, genre, language)

    conn = psycopg2.connect(dbname= 'moviebook', user='postgres', password='heybuddy', host='localhost', port='5432')
    cur = conn.cursor()
    query = '''INSERT INTO movies values (%s,%s,%s,%s);'''
    cur.execute(query,(name,actor,genre,language))
    conn.commit()
    conn.close()
    print('data added')
    new_window.destroy()
...
searchbutton = Button(text= 'search', command=searchFunction)
